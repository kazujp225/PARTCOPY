...
このプロジェクトの画像は元サイトからコピーしたものです。**必ず自社の画像に差し替えてください。**

#### 差し替え手順：
1. \`public/assets/\` に新しい画像を配置
2. 同じファイル名で上書きすればコード変更不要
3. ファイル名を変えたい場合は、コンポーネント内の \`src=\` や \`url()\` も更新

#### 差し替えのコツ：
- アスペクト比は元画像に合わせるとレイアウトが崩れない
- ロゴ・アイコンはSVG推奨
- 写真は元と同じサイズ以上を推奨（縮小は問題なし、拡大はぼやける）

\`\`\`
public/assets/ の画像を全てリストアップして、
各画像がどのコンポーネントで使われているか一覧表にしてください。
その後、指定する画像を差し替えます。
\`\`\`
...
`
    ...
    const imageManifest = `# 画像一覧 — 差し替えガイド

このプロジェクトに含まれる画像ファイルの一覧です。
**これらは元サイトからコピーした画像なので、自社の画像に差し替えてください。**

## 差し替え方法
同じファイル名で \`public/assets/\` に上書き配置するだけでOKです。
